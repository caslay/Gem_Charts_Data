  ...
  return (
    <div className="border border-slate-800/50 bg-slate-900/30 backdrop-blur-sm rounded-lg p-5">
      <div className="flex items-center justify-between mb-4">
        ...
        <span className="px-2 py-0.5 rounded text-[9px] bg-slate-800 text-slate-300 font-mono font-bold">
          {scans.length} SCANS
        </span>
      </div>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-12 border border-dashed border-slate-800 rounded-lg text-slate-500 font-mono text-[11px] text-center p-4">
          ...
          <span className="text-[9px] text-slate-600 mt-1">Configure lookback parameters and run a deep scan.</span>
        </div>
      ) : (
            ...
            return (
              <div
                ...
                className={`group cursor-pointer border rounded-lg p-3.5 transition text-left flex flex-col justify-between ${
                  isSelected
                    ? "border-cyan-500/60 bg-cyan-950/15 shadow-sm shadow-cyan-500/10"
                    : "border-slate-800/60 bg-slate-900/40 hover:bg-slate-900/80 hover:border-slate-700"
                }`}
              >
                <div className="flex items-start justify-between mb-2">
                  <div>
                    <h3 className="text-xs font-mono font-bold text-slate-200 uppercase tracking-tight group-hover:text-cyan-400 transition">
                      {scan.scan_name}
                    </h3>
                    <span className="text-[9px] text-slate-500 font-mono flex items-center gap-1.5 mt-0.5">
                      <span className="text-cyan-400/90 font-bold">{scan.symbol}</span>
                      <span>•</span>
                ...
                <div className="grid grid-cols-4 gap-1.5 border-t border-slate-800/40 pt-2.5 mt-1 font-mono text-[9px] text-center">
                  <div className="flex flex-col">
                    <span className="text-slate-500 uppercase text-[8px]">Setups</span>
                    <span className="font-bold text-slate-200">{scan.total_detected}</span>
                  </div>
                  <div className="flex flex-col">
                    <span className="text-slate-500 uppercase text-[8px]">Reclaim %</span>
                    <span className="font-bold text-cyan-400">{reclaimRate.toFixed(0)}%</span>
                  </div>
                  <div className="flex flex-col">
                    <span className="text-slate-500 uppercase text-[8px]">Retest %</span>
                    <span className="font-bold text-slate-300">{Number(scan.retest_rate_pct).toFixed(0)}%</span>
                  </div>
                  <div className="flex flex-col">
