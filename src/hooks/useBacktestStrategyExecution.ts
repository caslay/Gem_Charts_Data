...
export function useBacktestStrategyExecution({
  ...
  useEffect(() => {
    ...
    const scanConfig: SweepReclaimScanConfig = {
      ...
      volumeExpansionThreshold: config.volumeExpansionThreshold ?? 1.35,
      deltaDominanceThreshold: config.deltaDominanceThreshold ?? 52.0,
      bodyRatioThreshold: config.bodyRatioThreshold ?? 0.50,
      ...
      stage2Multiple: config.stage2Multiple ?? 1.4,
      stage3Multiple: config.stage3Multiple ?? 3.0,
      entryMode: config.entryMode ?? 'FVG_PROXIMAL',
      ...
      slBufferAtrMultiplier: config.slBufferAtrMultiplier ?? 0.12,
    };
