...
- **The Fixes:**
### 66. Runaway Polling Loop & Browser Socket Exhaustion (Resolved in V17.25)
- **The Bug:** The browser console flooded with repeated `Failed to load resource: net::ERR_INSUFFICIENT_RESOURCES` and `[useAutomatedStrategyExecution] Daemon state sync warning: TypeError: Failed to fetch`. Chrome ran out of network socket connections, freezing client-side navigation and breaking Google Sign-In on `/login` with `[auth][error] MissingCSRF`.
- **The Causes:**
  1. **Unstable Dependency Triggering Infinite Polling:** In `useAutomatedStrategyExecution.ts`, `fetchDaemonState` included `[livePrice, dispatchAlert]` in its `useCallback` dependency array. `livePrice` was provided by `useMarketDataLiveContext()`, which emits every time a WebSocket tick arrives (several times per second). On every tick, `fetchDaemonState` received a new reference, re-triggering `useEffect`, clearing the timer, and immediately executing an HTTP `fetch('/api/daemon/state')`.
  2. **Socket Exhaustion Without Cancellation:** Previous in-flight HTTP requests were not cancelled before firing the next request, piling up dozens of concurrent connections and exhausting the browser's HTTP socket pool (`ERR_INSUFFICIENT_RESOURCES`).
  3. **Un-Gated Providers on `/login` and Unauthenticated States:** `MarketDataProvider` in `layout.tsx` unconditionally mounted `useBinanceWS`, `useMarketData`, `AutonomousExecutionHost`, and `useAutoTradeExecutor` on all routes, including `/login`, and even when session status was `unauthenticated` or `loading`.
- **The Fixes:**
  1. **Decoupled `livePrice` via In-Memory `useMemo`:** In `useAutomatedStrategyExecution.ts`, `fetchDaemonState` now stores raw daemon positions in state (`rawActivePositions`), and `activePositions` computes `unrealizedR` and `unrealizedUsd` synchronously in memory via `useMemo([rawActivePositions, livePrice])`. Zero network fetches occur on price ticks.
  2. **In-Flight Cancellation (`AbortController`):** Attached an `AbortController` to cancel pending in-flight requests before firing a new poll or when unmounting. Silently ignore `AbortError` without triggering log noise or retries.
  3. **Stable Cadence & Graceful Error Damping:** Throttled `/api/daemon/state` to a stable 4000ms `setInterval` without reactive triggers. Failed attempts log a controlled warning and wait for the next timer tick.
  4. **Strict Route & Auth Gating:** In `MarketDataContext.tsx`, `useMarketData.ts`, and `useAutomatedStrategyExecution.ts`, all WebSocket connections, REST polling loops, autonomous execution hosts, and trade monitors are gated behind `isEnabled = authStatus === 'authenticated' && pathname !== '/login'`. When on `/login` or unauthenticated, background network activity drops to 0.

### 67. Circular Context Re-render Loop & 5.4GB Memory Leak Elimination (Resolved in V17.26)
- **The Bug:** Chrome memory ballooned to over 5.4 GB, the React Scheduler thrashed on infinite `postMessage` queueing, and all client pages (`/`, `/settings`, `/quant-lab`) froze on loading spinners with `AbortError` and stalled network promises. Mounting the chart logged `Cannot read properties of undefined (reading 'startTime')`.
- **The Causes:**
  1. **Unmemoized Return in `useMarketData` Breaking Context Memoization:** `useMarketData` returned an unmemoized object literal. Because `useBinanceWS` updates `liveCandle` and `livePrice` on every WebSocket tick (5-10 times/second), `MarketDataProvider` re-rendered, producing a brand-new `marketData` reference. Even though `staticValue` used `React.useMemo(..., [marketData, ...])`, the unmemoized reference caused `staticValue` to change on every tick, broadcasting re-render cascades to all subscribers (`AutonomousExecutionHost`, `Home`, `Chart`, `ThemeSync`, etc.) at tick frequency.
  2. **Un-diffed `setState` Invocations in Consumer Hooks:** Hooks (`useAutomatedStrategyExecution`, `useLiveOrderBlockExecution`, and `useMarketData`) called `setState` unconditionally with newly allocated array or object literals (e.g. `setRawActivePositions(data.activePositions || [])`, `setScannedSetups(res.scannedSetups)`, `setActiveZones([...])`, `setStructureState({ ...prev })`), causing React to trigger downstream re-renders even when underlying data was 100% identical.
  3. **Duplicate Background Execution Engines on Main Cockpit:** `AutonomousExecutionHost` in `MarketDataContext.tsx` was running `useAutomatedStrategyExecution()` and `useLiveOrderBlockExecution()` unconditionally. On `/`, `Home`, `AutomatedExecutionHUD`, and `LiveOrderBlockExecutionHUD` also ran their own engine instances, resulting in 4 parallel engines running concurrent timers and candidate scans.
  4. **Observer Race During Mount:** Event-loop thrashing caused React 19's Performance / Resource Timing entries to mutate concurrently during mount, throwing unhandled `TypeError: Cannot read properties of undefined (reading 'startTime')`.
- **The Fixes:**
  1. **Decoupled WebSocket Ticks & Memoized Market Data Return:** Removed `liveCandle` and `livePrice` ticks from being passed into `useMarketDataHook` in `MarketDataContext.tsx`. Wrapped `useMarketData`'s return object in `useMemo` so context consumers receive a strictly stable object reference that only changes when actual data/settings change.
  2. **Strict State Diffing Across Execution Hooks:** Added content equality checks (`JSON.stringify(prev) === JSON.stringify(next)`) across `setRawActivePositions`, `setPendingOrders`, `setClosedTrades`, `setScannedSetups`, `setActivePositions`, `setActiveZones`, `setActiveZonesByTimeframe`, and `setTestingStates`. In `useMarketData.ts`, `setStructureState` returns `prev` if dealing range statuses haven't transitioned.
  3. **Gated `AutonomousExecutionHost`:** Gated `AutonomousExecutionHost` to non-cockpit routes (`pathname !== '/'`), letting `Home` serve as the sole execution host on `/` and eliminating duplicate engines, duplicate scans, and duplicate poll intervals.
  4. **Chart Dependency Optimization & Defensive Observer Guards:** In `Chart.tsx`, wrapped `updateSvgCoordinates` to use `openTradesRef` and `srOverlayRef` with primitive dependency arrays; added candle content equality checks in `setLocalCandles`; guarded `setWsInterval`; and wrapped `ResizeObserver` with null-safe try/catch and added a global window error interceptor to prevent `startTime` observer crashes.

### 68. Lookahead Bias in Reclaim Displacement FVG Detection & In-Flight Position Sequential Walking (Resolved in V17.30)
- **The Bug:** On `2026-09-03`, Quant Lab historical replay displayed 7 executed trades with phantom losses and wrong fills, while the live PM2 headless engine recorded strictly 3 closed trades (+0.70R Net Profit, 66.7% Win Rate: 1 Loss, 1 Full TP2 Win, 1 Stage 1 Scratch) plus 1 active overnight in-flight Short position (`$2511.28`).
- **The Causes:**
  1. **Displacement FVG Lookahead Bias:** `SweepReclaimEngine.ts` evaluated `const searchMax = Math.min(n - 1, i + 2)` when checking for displacement Fair Value Gaps. In historical backtest with 1000 bars, this peeked 2 candles into the future beyond reclaim close `i`, finding FVGs that did not yet exist when candle `i` closed in real-time execution. This artificially shifted entry prices and target levels (e.g. $2422.46).
  2. **Missing Reclaim Candle Immediate Expansion Check:** When a violent displacement reclaim candle closed at or beyond Target 1 (`reclaimClose <= target1` for Shorts or `>= target1` for Longs), the move had already transpired before a limit order could be placed. Live PM2 rejected these immediately (`MISSED_TP1_EXPANSION`), but backtest naively searched subsequent bars for retests.
  3. **In-Flight Position Blindness in Sequential Walk:** In `equityCalculator.ts`, `nonExecutionOutcomes` included `'PENDING'`. Active in-flight positions (such as the overnight Short @ $2511.28) were filtered out of `candidateSetups`, causing the sequential walk to be blind to the open short. Consequently, an opposing Long setup at 23:40 Cairo ($2506.37) was falsely executed in Quant Lab (recording a -1.00R loss) when real-world PM2 Guardrail 2 & 3 vetoed it.
  4. **Anchor Tie-Breaking Discordance:** When multiple anchors triggered on the same displacement wave with identical retest timestamps (e.g. 10:20 Cairo Asian High $2409.60 vs Swing Pivot $2408.53), `equityCalculator.ts` tie-broke strictly on static anchor tier ranking rather than price proximity to market, causing divergent setup selection.
- **The Fixes:**
  1. **Strict Reclaim Horizon (`searchMax = i`):** Enforced that displacement FVGs can only be formed by candles that have closed on or before reclaim candle `i` (`const searchMax = i;`), eliminating lookahead bias.
  2. **Phase 4 Immediate Missed Expansion Invalidation:** If `(isBullish && reclaimClose >= target1) || (!isBullish && reclaimClose <= target1)`, mark `status = 'RECLAIMED_NO_RETEST'`, `simulated_outcome = 'NO_RETEST'`, bailing out before the retest loop.
### 69. Intra-Candle Sequence Bug in Early Breakeven Simulator & Real-World Live Parity (Resolved in V17.31)
- **The Bug:** Running a backtest scan in Quant Lab with Early Breakeven active yielded a catastrophic -91.04% drawdown ($896.15 remaining from $10,000) with a 16.3% win rate and -109.85R net loss, whereas theoretical post-facto calculation had projected a massive positive curve.
- **The Causes:**
  1. **Same-Bar Stop-Out Corruption:** When price touched the limit entry and expanded to +0.60R MFE on the SAME 5-minute candle, `SweepReclaimEngine.ts` ratcheted `activeStopLoss = entryPrice` and set `stageFilledThisBar = true`. Line 2224 evaluated `if (low <= checkSL)`. Because the candle's low had naturally dipped to fill the limit entry, the simulator falsely assumed price surged first and then crashed, immediately terminating the trade at Breakeven on the exact same candle it entered (`retest_time === exit_time`). Exactly 609 winning trades were prematurely murdered on their entry bar!
  2. **Theoretical Post-Facto vs. Real-World Live Execution Disparity:** The theoretical post-facto model assumed that all 623 baseline TP2 winners would still reach TP2 while only losers would be scratched. In real live trading on Binance Futures with a resting Breakeven stop, ~150 trades that reached +0.60R dipped back to entry on subsequent bars before continuing to TP2, causing them to be legitimately scratched at 0.00R rather than harvesting +1.20R.
- **The Fixes:**
  1. **Fixed In-Scanner Simulation Engine:** In `SweepReclaimEngine.ts`, removed `stageFilledThisBar = true` upon Early Breakeven ratchet for both Bullish (line 2161) and Bearish (line 2280) logic. On bar $i$, the trade cannot be stopped out by the dip that preceded the expansion. The new Breakeven stop loss takes effect strictly starting on bar $i + 1$, slashing same-bar false exits from 609 down to 1!
  2. **Natively Integrated Factory Preset:** Registered `factory_sr_5m_alpha_shield_early_be` ("5m Sweep & Reclaim Alpha Shield (Early BE + Wave Guard)") with +0.50R MFE Breakeven, Wave Deduplication, and 45m Post-Loss Cooldown. Under exact candle-by-candle path simulation, it delivers **+161.4R Net Profit**, **1.37 Profit Factor**, a minimal **-13.20R Max Drawdown**, and grows a $10,000 account to **$202,853.20 (+1,928% Net Compounded Return)** with only 24% drawdown — 100% executable in real-world live trading.

### 70. The "Throwing Traders to the Lions" Post-Mortem: The Zero-Guessing 100% Quant Lab & Live Execution Parity Mandate (Resolved in V17.32)
- **The Dangerous Incident:**
  An AI agent computed a theoretical $+12,435,973.43$ compounded return by applying post-facto ledger filters to historical trades, presenting it to the trader as an achievable trading strategy. When the trader loaded this exact preset into Quant Lab to test it under real simulation, the backtest crashed into a catastrophic **$-91.04\%$ drawdown** ($\$896.15$ remaining from $\$10,000$, $16.3\%$ Win Rate). The trader rightly called out: *"okay u was wrong and was going to throw me to the lions my friend... make sure the Quant Lab (it built to test) is matching the live execution 100% no guessing anymore."*
- **The 4 Cardinal Mistakes & Root Causes:**
  1. **The Fallacy of Asymmetric Scratches (Post-Facto Ledger Slicing):** In offline spreadsheet or post-hoc analysis, it is easy to assume that when a rule like *"move stop to breakeven at $+0.60\text{R}$"* is engaged, only losing trades will be converted into scratches, while $100\%$ of winning trades will remain full $+1.20\text{R}$ winners. In real live market physics on Binance Futures, price action is bidirectional. A resting Breakeven stop order placed on the exchange cannot tell whether a trade will eventually reach TP2; if price pulls back to entry, the order triggers and closes at $0.00\text{R}$. Approximately $\sim 150$ winning trades undergo normal intra-trade pullbacks to entry before expanding to TP2. By assuming zero winners get scratched, the model hallucinated $\$12.4\text{M}$ when true path execution delivers $+161.4\text{R} \to \$202,853$.
  2. **Intra-Candle Time-Travel Sequence Inversion:** Within a single 5m bar, OHLC is a summary, not a chronological tape. For a limit entry, price dips to entry (forming the low), fills the order, and then rallies toward the high. If the engine detects $+0.60\text{R}$ MFE on that candle and ratchets Stop Loss to entry, and then in that same candle checks `low <= activeStopLoss`, the simulator falsely assumes price hit the high first and then crashed below entry. This killed 609 winning trades on their entry bar!
  3. **Presenting Unverified Post-Facto Projections to the Trader:** An AI agent must NEVER communicate a post-facto calculation as a real trading strategy. Quant Lab exists for the exact purpose of running path-dependent simulations. If a number was not generated by the actual end-to-end Quant Lab engine across raw historical candles, it does not exist.
  4. **Lack of Pre-Flight In-Scanner Verification:** Before any preset is recommended or registered as a "winning setup", the agent must run the actual in-scanner backtest in Quant Lab to observe the real equity curve, drawdown, and trade list.
- **The Inviolable "Zero-Guessing 100% Parity Mandate":**
  1. **Quant Lab $\equiv$ PM2 Live Execution:** Quant Lab is the exact digital twin of live execution. Every rule, guardrail, order type, TTL, fill logic, and stop ratchet must match bit-for-bit. Zero guessing, zero approximations, zero divergence.
  2. **Strict Next-Bar Ratchet Rule:** Any stop-loss modification or breakeven ratchet triggered on candle $i$ takes effect strictly on candle $i + 1$. Same-bar stop evaluations against the entry candle's fill dip are strictly prohibited.
  3. **Prohibition of Post-Facto Trade Ledger Modification:** An AI agent is permanently forbidden from modifying trade outcomes in memory (e.g. `if (s.mfe >= 0.60) s.realized_rr = 0.0`) to fabricate performance figures. Strategy performance must be derived purely from sequential, path-dependent candle-by-candle simulation.
  4. **The Principle of Institutional Truth:** Always communicate the verified path-dependent reality ($+161.4\text{R}$, $1.37\text{ PF}$, $-13.2\text{R DD}$, $\$202,853$ equity) with total honesty. Real trading on Binance Futures requires knowing the exact drawdown, scratch frequency, and execution friction.

### 71. Early Breakeven Event Disambiguation & Breakeven Scratch Accounting Fix (Resolved in V17.39)
- **The Bug:** When Rule 4 (Early Breakeven at $\ge +0.40\text{R}$ MFE) triggered on a live trade (e.g. ETHUSDC 5m long at $2452.53 on 2026-09-04 20:45 UTC), Telegram dispatched a false alert *"🎯 [STAGE 1 HARVEST] Take Profit 1 reached @ $2456.21"*, even though price only reached $2454.34 (+0.44R) and never touched TP1. Furthermore, when price subsequently pulled back and stopped out at Breakeven ($2452.53), the trade was falsely recorded as a full `-1.00R` / `-$300.00` loss (`STOPPED_OUT`), corrupting the performance ledger and triggering unwarranted risk circuit breaker warnings.
- **The Causes:**
  1. **Event Identifier Collision in Engine:** In `AutomatedStrategyExecutionEngine.ts` (line 918), the early breakeven ratchet emitted `"STAGE_1_HARVEST"` instead of a dedicated event. Downstream subscribers (Telegram notifier, WebSocket broadcast) naturally reported it as a Take Profit 1 harvest.
  2. **Inverted Exit Classification:** In `closePosition()`, the logic checked `if (!pos.isStage1Filled && !pos.isStage2Filled)`. Because TP1 was not genuinely filled, the trade defaulted to `STOPPED_OUT` with full `-1.0R` loss, even though `activeStopLoss` was already securely ratcheted to the entry price.
- **The Fixes:**
  1. **Dedicated `EARLY_BREAKEVEN` Event:** Introduced `EARLY_BREAKEVEN` in `ExecutionEventType` and `DaemonSessionEvent`. Line 918 now emits `EARLY_BREAKEVEN`, and Telegram displays `🛡️ [EARLY BREAKEVEN ACTIVATED — RULE 4]` showing floating MFE and new BE stop level.
  2. **Dedicated `BREAKEVEN_SCRATCH` Exit Reason:** In `closePosition()`, if `pos.trailingSlSource === "BREAKEVEN"` or $|exitPrice - entryPrice| \le 0.05$, the trade is categorized as `BREAKEVEN_SCRATCH` with `realizedR = 0.0` and `realizedUsd = 0.0`, achieving 100% parity with `SweepReclaimEngine.ts:L2249-2254`.
  3. **Ledger & DB Reconciliation:** Reconciled historical logs in `ETHUSDC_Daily_Tracker.json` and PostgreSQL `trades` table on VPS (`quegar_db`).

### 72. Intra-Candle Retest Fill Priority & Structural Dealing Range Dynamic Parity (Resolved in V17.40)
- **The Bug:** On 2026-09-04 20:45 UTC to 22:05 UTC, the live PM2 daemon executed 3 trades (Long $2452.53 $\to$ Scratch, Short $2455.15 $\to$ +1.12R Win, Long $2451.84 $\to$ Scratch). When running a backtest scan in Quant Lab on `localhost:4000` or the live VPS, the first two trades were completely missing from the trade table, jumping directly from 19:40 to 01:05.
- **The Causes:**
  1. **Intra-Candle Pre-Fill Invalidation Race Condition (`SweepReclaimEngine.ts`):** In Phase 4 retest search, the engine evaluated pre-fill invalidation (missed TP1 expansion: `(open <= executionEntry && low <= target1)`) *before* checking whether the candle touched the resting limit entry (`high >= executionEntry`). On the 21:50 UTC candle, open was $2454.24, high reached $2455.55 (touching limit $2455.15), and low dropped to $2449.20 (clearing TP1 $2451.47 and TP2 $2450.36). Because `open <= executionEntry` and `low <= target1` were both true, the code falsely assumed price reached TP1 *before* touching entry and aborted as `NO_RETEST`, ignoring that the high penetrated the limit order.
  2. **Structural Dealing Range Valuation Parity Gap (`route.ts` & `SweepReclaimEngine.ts`):** The live PM2 daemon derives its dealing range from `MarketStructureAPI.analyze()`, which tracks Level 2 (MAJOR) Protected Anchors ($2463.99 / $2441.61 $\to$ EQ $2452.80$). In Quant Lab, `structuralDealingRange` was omitted from API request bodies, causing the engine to fall back to an arbitrary local lookback that produced EQ $2451.205$, causing Setup $2452.53$ to be falsely rejected as "in premium" (`is_valuation_aligned: false`).
- **The Fixes:**
  1. **Retest Fill Evaluated First:** In `SweepReclaimEngine.ts:L2006-2065`, the limit fill condition (`low <= executionEntry` for longs, `high >= executionEntry` for shorts) is now checked FIRST. Pre-fill target invalidation only triggers if price reached Target 1 *without* touching entry (`high < executionEntry` for shorts, `low > executionEntry` for longs).
  2. **Dynamic Route Alignment (`route.ts`):** In `src/app/api/quant-lab/sweep-reclaim-scanner/route.ts`, if `structuralDealingRange` is omitted from the request body, the route dynamically derives it from `MarketStructureAPI.analyze(candles, lastPrice)`, guaranteeing bit-for-bit parity with PM2 execution.
  3. **Causal Structural Pivot Fallback:** Hardened `SweepReclaimEngine.ts` pivot fallback to evaluate confirmed pivots up to `evalIndex` (`reclaimIdx`) prioritizing Level 2 (MAJOR) swings.
...
### 74. The Taker Fee Drag Illusion & Double-Deduction Phantom Fee Bug (Resolved in V17.47)
- **The Bug:** When testing Sweep & Reclaim setups with Fee-Padded Breakeven (Taker Shield) enabled at 0.04% offset in Quant Lab, the strategy backtest showed a severe net loss (`-57.67R Net`, `$252.57` remaining from `$1,000`), while disabling the fee shield showed a solid profit (`+52.27R Net`, `$6,621.00`). The trader rightly flagged: *"this make no sense, while disable the fee sheild it show proft !! u have to double audit it all again"*.
- **The Causes:**
  1. **Double-Deduction Phantom Fee Accounting:** Under Fee-Padded Breakeven, the stop loss is padded by `Entry * (1 ± offset)` specifically so that when the trailing stop hits, the price appreciation (`+$2.30` or `+0.23R`) covers the exchange taker fee (`-$2.30` or `-0.23R`), resulting in exactly `$0.00` net cash drag. However, `equityCalculator.ts` treated these scratches as unshielded exits and deducted the fee a second time (`-0.23R` per trade). Across 1,288 scratches in a 1-year test, the calculator hallucinated **`-296.24R` in phantom losses**, turning a profitable strategy into an artificial drawdown.
  2. **Intra-Bar Retest Breathing Room Violation:** Moving the stop loss tighter immediately upon reaching the early breakeven threshold without ensuring price had moved sufficiently beyond the fee offset caused normal intra-bar noise to prematurely stop out 87 winning trades before they could expand to Target 2, cutting gross returns from `+383.4R` to `+300.26R`.
- **The Fixes:**
  1. **Dynamic Breathing Room Guard (`SweepReclaimEngine.ts` & `AutomatedStrategyExecutionEngine.ts`):** Enforced that the early breakeven ratchet only engages once price reaches at least `effectiveEarlyBEMultiple = Math.max(earlyBreakevenMultiple, feeOffsetInR + 0.05)`, guaranteeing at least $+0.05\text{R}$ of real breathing room above the fee offset.
