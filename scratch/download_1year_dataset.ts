...
async function main() {
  ...
  // 1 Full Year: 2025-08-27 00:00 UTC to 2026-08-27 18:00 UTC (365 Days)
  const startMs = Date.parse('2025-08-27T00:00:00.000Z');
  const endMs = Date.parse('2026-08-27T18:00:00.000Z');

  console.log(`Downloading 1-YEAR dataset for ${symbol} ${interval} from ${new Date(startMs).toISOString()} to ${new Date(endMs).toISOString()}...`);
  const candles = await fetchPagedKlines(symbol, interval, startMs, endMs);
  
  console.log(`Total 1-year candles downloaded: ${candles.length}`);
  ...
  if (candles.length > 0) {
    ...
    console.log(`1-Year Dataset cached successfully to ${cachePath} (${(fs.statSync(cachePath).size / 1024 / 1024).toFixed(2)} MB)`);
    ...
    console.log('\n--- 1-Year Market Characterization ---');
