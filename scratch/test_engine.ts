import { MarketStructureEngine } from '../src/lib/structureEngine';
import type { Candle } from '../src/lib/fvgEngine';
...
interface StructuralDealingRange {
  ...
  low: number;
  equilibrium: number;
  current_status: 'PREMIUM' | 'DISCOUNT' | 'UNKNOWN';
...
function buildZigZagSegments(
  swingsOnly: StructuralSwing[],
  events: any[],
  isDisplacement: boolean
) {
  const segments: ZigZagSegment[] = [];
  let trendState: 'BULLISH' | 'BEARISH' | 'UNSET' = 'UNSET';
  let latestMSSegment: ZigZagSegment | null = null;
  ...
  for (let i = 0; i < swingsOnly.length - 1; i++) {
    ...
    let trendAfter: 'BULLISH' | 'BEARISH' | 'UNSET' = trendState;
...
async function runTest() {
  try {
    ...
    const majorEngine = new MarketStructureEngine();
    ...
    const innerEngine = new MarketStructureEngine({
      adaptiveNMin: 1,
      adaptiveNMax: 2
    });
    for (const c of normalizedCandles) {
    ...
    const rawSwings: StructuralSwing[] = majorEngine.confirmed_pivots.map(pt => ({
      t: pt.timestamp,
      price: pt.price,
    ...
    for (const s of confirmedSwings) {
      ...
      if (last.type === s.type) {
        if (s.type === 'HIGH') {
          if (s.price > last.price) alternatingSwings[alternatingSwings.length - 1] = s;
        } else {
          if (s.price < last.price) alternatingSwings[alternatingSwings.length - 1] = s;
        }
      } else {
    ...
    const rawInnerSwings: StructuralSwing[] = innerEngine.confirmed_pivots.map(pt => ({
      t: pt.timestamp,
      price: pt.price,
      type: pt.type === 'SWING_HIGH' ? 'HIGH' : 'LOW',
      grade: 'INNER',
    ...
    const alternatingInnerSwings: StructuralSwing[] = [];
    for (const s of confirmedInnerSwings) {
      ...
      if (last.type === s.type) {
        if (s.type === 'HIGH') {
          if (s.price > last.price) alternatingInnerSwings[alternatingInnerSwings.length - 1] = s;
        } else {
          if (s.price < last.price) alternatingInnerSwings[alternatingInnerSwings.length - 1] = s;
        }
      } else {
