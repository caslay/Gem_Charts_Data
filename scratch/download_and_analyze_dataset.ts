...
async function main() {
  ...
  // Period: 2026-03-01 to 2026-08-26 (~6 months, covering diverse regimes)
  const startMs = Date.parse('2026-03-01T00:00:00.000Z');
  const endMs = Date.parse('2026-08-26T18:00:00.000Z');

  console.log(`Downloading ${symbol} ${interval} dataset from ${new Date(startMs).toISOString()} to ${new Date(endMs).toISOString()}...`);
  const candles = await fetchPagedKlines(symbol, interval, startMs, endMs);
  
  console.log(`Total candles downloaded: ${candles.length}`);
  
  if (candles.length > 0) {
    const cachePath = path.join(__dirname, 'candles_5m_ethusdc.json');
    fs.writeFileSync(cachePath, JSON.stringify(candles));
    console.log(`Dataset cached successfully to ${cachePath} (${(fs.statSync(cachePath).size / 1024 / 1024).toFixed(2)} MB)`);
    ...
    console.log('\n--- Market Period Characterization ---');
    console.log(`Start Date: ${new Date(candles[0].t).toISOString().slice(0, 10)} @ $${startPrice.toFixed(2)}`);
    console.log(`End Date:   ${new Date(candles[candles.length - 1].t).toISOString().slice(0, 10)} @ $${endPrice.toFixed(2)}`);
    ...
    console.log(`Price Range Expansion: $${(maxPrice - minPrice).toFixed(2)} (${(((maxPrice - minPrice) / minPrice) * 100).toFixed(1)}% amplitude)`);
    console.log('----------------------------------------\n');
  }
}
